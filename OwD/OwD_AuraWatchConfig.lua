...
local Spell_Card = function(f)
	...
	f.Icon = L.create_Texture(f, "ARTWORK", "Icons\\Reconfigure", 24,24, 0,1,0,1, C.Color.White,0.9, "LEFT",f,"LEFT",6,0)
	f.Icon_Extra = L.create_Texture(f, "OVERLAY", "Right_IconForbid0", 24,24, 0,1,0,1, C.Color.Red,0.9, "CENTER",f.Icon,"CENTER",0,0)
	...
	f.Delete.Tex = L.create_Texture(f.Delete, "ARTWORK", "ConfigNo", 18,18, 0,1,0,1, C.Color.White,0.9, "CENTER",f.Delete,"CENTER",0,0)
...
local function update_IconExtra(f,classFileName,specID,i)
	if OwD_DB.AuraFilter[classFileName][specID][f[i].Indicator][i].Show then
		if OwD_DB.AuraFilter[classFileName][specID][f[i].Indicator][i].Show == "show" then
			f[i].Icon_Extra: Hide()
		else
			f[i].Icon_Extra: Show()
		end
	else
		OwD_DB.AuraFilter[classFileName][specID][f[i].Indicator][i].Show = "show"
		f[i].Icon_Extra: Hide()
	end
end
...
L.update_Card = function(f)
	...
	if not OwD_DB.AuraFilter[classFileName][specID][3] then
		OwD_DB.AuraFilter[classFileName][specID][3] = {}
	end
	n3 = #OwD_DB.AuraFilter[classFileName][specID][3]
	if #f.Card >= n1 + n2 + n3 + 1 then
		for i = n1 + n2 + n3 + 1, #f.Card do
			if f.Card[i] then
				f.Card[i]: Hide()
				f.Card[i] = nil
			end
	...
	if n1 + n2 + n3 >= 1 then
		for i = 1, n1 + n2 + n3 do
			...
			elseif i > n1 and i <= n1+n2 and (OwD_DB.AuraFilter[classFileName][specID][2][i-n1].Icon) then
				...
				f.Card[i].Indicator3: SetVertexColor(unpack(C.Color.White2))
			elseif i > n1+n2 and i <= n1+n2+n3 and (OwD_DB.AuraFilter[classFileName][specID][3][i-n1-n2].Icon) then
				f.Card[i].Icon: SetTexture(F.Media.."Icons\\"..OwD_DB.AuraFilter[classFileName][specID][3][i-n1-n2].Icon)
				f.Card[i].Spell: SetText(GetSpellInfo(OwD_DB.AuraFilter[classFileName][specID][3][i-n1-n2].Spell))
				f.Card[i].Aura: SetText(OwD_DB.AuraFilter[classFileName][specID][3][i-n1-n2].Aura)
				f.Card[i].Unit: SetText(OwD_DB.AuraFilter[classFileName][specID][3][i-n1-n2].Unit)
				f.Card[i].Indicator = 3
				f.Card[i].Indicator1: SetVertexColor(unpack(C.Color.White2))
				f.Card[i].Indicator2: SetVertexColor(unpack(C.Color.White2))
				f.Card[i].Indicator3: SetVertexColor(unpack(C.Color.Blue))
			end
			update_IconExtra(f.Card,classFileName,specID,i)
			...
			f.Card[i]: SetScript("OnMouseDown", function(self,button)
				...
				elseif button == "MiddleButton" then
					if OwD_DB.AuraFilter[classFileName][specID][f.Card[i].Indicator][i].Show == "show" then
						OwD_DB.AuraFilter[classFileName][specID][f.Card[i].Indicator][i].Show = "hide"
						L.update_Card(f)
					else
						OwD_DB.AuraFilter[classFileName][specID][f.Card[i].Indicator][i].Show = "show"
						L.update_Card(f)
					end
				end
			end)
			f.Card[i].Delete: SetScript("OnMouseDown", function(self,button)
				...
				elseif i > n1+n2 and i <= n1+n2+n3 then
					table.remove(OwD_DB.AuraFilter[classFileName][specID][f.Card[i].Indicator],i-n1-n2)
				end
				L.update_Card(f)
			end)
		end
	...
	if n1+n2+n3 >= 1 then
		...
		f.Explain: SetPoint("TOPLEFT", f.Card[n1+n2+n3], "BOTTOMLEFT", 0,-4)
	else
		f.Explain: ClearAllPoints()
...
local init_AW_Config = function(f)
	...
	f.Explain: SetSize(f:GetWidth(),40)
	f.Explain: SetPoint("TOPLEFT", f, "BOTTOMLEFT", 0,-4)
	...
	f.Explain.Text: SetWidth(380)
	f.Explain.Text: SetPoint("LEFT", f.Explain, "LEFT", 4,2)
	f.Explain.Text: SetText(L.Text["CONFIG_AuraWatch_INTRODUCE"])
	...
	f.insert.Indicator: SetScript("OnMouseDown", function(self,button)
		...
		elseif f.insert.Indicator.ID == 2 then
			f.insert.Indicator.ID = 3
		elseif f.insert.Indicator.ID == 3 then
			f.insert.Indicator.ID = 1
		end
		for i = 1,3 do
	...
	f.insert.Delete.Tex = L.create_Texture(f.insert.Delete, "ARTWORK", "ConfigYes", 18,18, 0,1,0,1, C.Color.White,0.9, "CENTER",f.insert.Delete,"CENTER",0,0)
	...
	f.insert.Delete: SetScript("OnMouseDown", function(self,button)
		...
		if f.insert.Icon.ID and f.insert.Indicator.ID then
			if ((f.insert.Spell.ID ~= "") and (f.insert.Spell.ID ~= " ")) or (((f.insert.Spell.ID ~= "") or (f.insert.Spell.ID ~= " ")) and f.insert.Aura.ID ~= "" and f.insert.Unit.ID) then
				...
				elseif f.insert.Indicator.ID == 3 then
					OwD_DB.AuraFilter[classFileName][specID][3][n3+1] = {
						Spell = f.insert.Spell.ID,
						Aura = f.insert.Aura.ID, 
						Unit = f.insert.Unit.ID, 
						Icon = f.insert.Icon.ID,
...
L.AuraWatch_Config = function(f)
	...
	f.AW_Config: SetPoint("LEFT", f, "CENTER", 240,80)
	...
	f.AW_Config: RegisterEvent("PLAYER_SPECIALIZATION_CHANGED")
	f.AW_Config: RegisterEvent("PLAYER_TALENT_UPDATE")
	f.AW_Config: SetScript("OnEvent", function(self, event)
		L.update_AW_Config(f.AW_Config)
	end)
	...
	f.AW_Config.Close.Tex = L.create_Texture(f.AW_Config.Close, "ARTWORK", "ConfigNo", 18,18, 0,1,0,1, C.Color.White,0.9, "CENTER",f.AW_Config.Close,"CENTER",0,0)
	...
	f.AW_Config.Reset.Warn.Yes.Tex = L.create_Texture(f.AW_Config.Reset.Warn.Yes, "ARTWORK", "ConfigYes", 18,18, 0,1,0,1, C.Color.White,0.9, "CENTER",f.AW_Config.Reset.Warn.Yes,"CENTER",0,0)
	...
	f.AW_Config.Reset.Warn.No.Tex = L.create_Texture(f.AW_Config.Reset.Warn.No, "ARTWORK", "ConfigNo", 18,18, 0,1,0,1, C.Color.White,0.9, "CENTER",f.AW_Config.Reset.Warn.No,"CENTER",0,0)
