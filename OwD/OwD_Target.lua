	...
	if event == "PLAYER_ENTERING_WORLD" or event == "PLAYER_TARGET_CHANGED" or event == "PLAYER_XP_UPDATE" or event == "UNIT_FACTION" then -->UNIT_FACTION is for Tapping
		if UnitExists("target") then
			...
			if level and level < 0 then
				f.Lv: SetText(L.Text["TARGET_CLASSIFICATION_BOSS"])
			else
				if classification == "elite" then
					f.Lv: SetText(level.." "..L.Text["TARGET_CLASSIFICATION_ELITE"])
				elseif classification == "rare" then
					f.Lv: SetText(level.." "..L.Text["TARGET_CLASSIFICATION_RARE"])
				elseif classification == "rareelite" then
					f.Lv: SetText(level.." "..L.Text["TARGET_CLASSIFICATION_RAREELITE"])
				elseif classification == "worldboss" then
					f.Lv: SetText(level.." "..L.Text["TARGET_CLASSIFICATION_WORLDBOSS"])
				else
					f.Lv: SetText(level)
				end
			end
